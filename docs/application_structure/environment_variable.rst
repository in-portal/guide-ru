...
+-----------+-------------------------------------------------------------------------------------------------------+
| ``event`` | `Событие <EventHandler:About>`__, которое требуется выполнить у данного :ref:`префикса <uc_Prefix>`.  |
+-----------+-------------------------------------------------------------------------------------------------------+
...
.. note::
   ...
   ``kEvent::getPrefixSpecial()`` (для `событий <EventHandler:About>`__) и ``TagProcessor:getPrefixSpecial()``
   (для `тэгов <TagProcessor:About>`__).
...
`тэгах <TagProcessor:About>`__ `m_Link <TagProcessor:m_Link>`__, `st_ContentBlock <TagProcessor:st_ContentBlock>`__,
`lang_LanguageLink <TagProcessor:lang_LanguageLink>`__ и `m_FormAction <TagProcessor:m_FormAction>`__. Этот метод
принимает 4 описанных ниже параметра.
...
|                       | .. warning::                                                                                        |
|                       |                                                                                                     |
|                       |    На текущий момент данная функциональность не работает                                            |
|                       |    (см. `этот вопрос <Вопрос:Как принудительно указать тегу m Link строить линк на Front-End?>`__)  |
|                       |    и чтобы построить ссылку на пользовательскую часть сайта нужно передать ``index.php`` в          |
|                       |    качестве значения параметра :ref:`url_index_file`.                                               |
+-----------------------+-----------------------------------------------------------------------------------------------------+
...
|    :type: string     |                                                                                              |
+----------------------+----------------------------------------------------------------------------------------------+
| .. config-property:: | Название `события <EventHandler:About>`__ для префикса ``cart``. Указанное событие           |
...
После успешного выполнения каждого `события <EventHandler:About>`__ происходит автоматическое перенаправление на шаблон,
...
+----------------------+---------------------------------------------------------------------------------------------------------------+
| .. config-property:: | :ref:`ID темы <tc_Theme_ThemeId>`, которую нужно использовать для показывания                                 |
|    :name: m_theme    | пользовательской части сайта. Значение данной переменной не используется в                                    |
|    :type: int        | административной консоли. Если не задать, то будет использовано                                               |
|    :ref_prefix: env_ | :ref:`ID основной темы <tc_Theme_PrimaryTheme>`, заданное в                                                   |
...
|    :name: m_opener   | (события :doc:`/event_description/events_in_temporary_tables/on_save`,                                        |
|    :type: int        | :doc:`/event_description/events_for_editing_records/on_create`,                                               |
|    :ref_prefix: env_ | :doc:`/event_description/events_for_editing_records/on_update`) и "``Cancel``"                                |
|                      | (события :doc:`/event_description/events_in_temporary_tables/on_cancel_edit`,                                 |
|                      | :doc:`/event_description/events_for_editing_records/on_cancel`) на панели инструментов на формах              |
|                      | редактирования автоматически происходил возврат на тот шаблон, с которого пользователь попал                  |
|                      | на эту форму редактирования. Для этого используется массив ``opener_stack_<m_wid>``, содержащий               |
|                      | шаблоны, заходя на которые пользователь в итоге попал на данный шаблон (напр.                                 |
|                      | ``Array ('users/user_list', 'users/user_edit_groups');``).                                                    |
|                      |                                                                                                               |
|                      | .. note::                                                                                                     |
|                      |                                                                                                               |
...
.. Broken Links:
   ...
   Вопрос:Как принудительно указать тегу m Link строить линк на Front-End?
